...
public interface SolvableItemDao extends BaseDao<SolvableItem> {
    ...
    @Query("SELECT * FROM solvableitem s JOIN clue c ON (s.id = c.clueSolvableItemId) JOIN attempt a ON (s.id = a.attemptSolvableItemId) WHERE c.clueId is not null AND s.nextDueDate < :time AND s.bucketId = :bucketId ")
    LiveData<List<SolvableTranslationCto>> getSolvableTranslationsDueBefore(Long bucketId, Instant time);
